...
<body>
    ...
    <div class="bodyRight category">
        ...
        <div class="btn-group article-btn" role="group" aria-label="..." >
            ...
            <a type="button" id="articleDelete" class="btn btn-default">删除</a>
        </div>
        <form class="form-horizontal">
            <table class="table table-bordered menu_manage" >
                <tr><th><input type="checkbox" class="select-all" />全选</th><th>id</th><th>名称</th><th>类别</th><th>描述</th><th>作者</th><th>创建时间</th><th>修改时间</th><th>状态</th><th>操作</th></tr>
                <?php if(is_array($articles)): $i = 0; $__LIST__ = $articles;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr><td><input type="checkbox" class="article-input" name="delete[]" data-uid="<?php echo ($vo["id"]); ?>" /></td><td><?php echo ($vo["id"]); ?></td><td><?php echo ($vo["title"]); ?></td><td> <?php if(is_array($types)): $i = 0; $__LIST__ = $types;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$type): $mod = ($i % 2 );++$i; if($type['id'] == $vo['pid']): echo ($type["title"]); endif; endforeach; endif; else: echo "" ;endif; ?></td><td><?php echo ($vo["description"]); ?></td><td><?php echo ($vo["author"]); ?></td><td><?php echo ($vo["creattime"]); ?></td><td><?php echo ($vo["modifytime"]); ?></td><td> <?php if($vo["status"] == 1 ): ?>开启 <?php else: ?> 关闭<?php endif; ?> </td><td><a href="/Admin/Article/edit/id/<?php echo ($vo["id"]); ?>">编辑</a> <a href="/Admin/Article/closeRecommend/id/<?php echo ($vo["id"]); ?>">关闭</a></td></tr><?php endforeach; endif; else: echo "" ;endif; ?>
            </table>
